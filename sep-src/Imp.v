...
Inductive bevalR : bexp -> bool -> Prop :=
	...
	| E_BEq : forall a1 a2 n1 n2, a1 || n1 -> a2 || n2 -> bevalR (BEq a1 a2) (beq_nat n1 n2)
	| E_BLe : forall a1 a2 n1 n2, a1 || n1 -> a2 || n2 -> bevalR (BLe a1 a2) (ble_nat n1 n2)
	| E_BNot : forall be bl, bevalR be bl -> bevalR (BNot be) (negb bl)
	| E_BAnd : forall be1 bl1 be2 bl2,
		bevalR be1 bl1 -> bevalR be2 bl2 -> bevalR (BAnd be1 be2) (andb bl1 bl2).

Theorem beval_iff_bevalR : forall be bl,
	bevalR be bl <-> beval be = bl.
Proof.
	split.
	Case "->".
		intros H; induction H; subst; try reflexivity;
		try (apply aeval_iff_aevalR in H; apply aeval_iff_aevalR in H0; subst; reflexivity).
	Case "<-".
		intros. destruct H. induction be.
		apply E_BTrue. apply E_BFalse.
		apply E_BEq; apply aeval_iff_aevalR; reflexivity.
		apply E_BLe; apply aeval_iff_aevalR; reflexivity.
		apply E_BNot in IHbe. simpl. apply IHbe.
		simpl. apply E_BAnd. apply IHbe1. apply IHbe2.
Qed.
...
(** The arithmetic and boolean evaluators can be extended to handle
...
Proof.
	...
	apply E_WhileLoop with (update (update (update (update (update (update
		empty_state X 2) Y 0) Y 2) X 1) Y 3) X 0). reflexivity.
	Case "2nd iteration".
...
Inductive no_whilesR: com -> Prop :=
	...
	| nw_ass : forall id a, no_whilesR (id ::= a)
	| nw_seq : forall c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (c1 ;; c2)
	| nw_ifb : forall b ct cf, no_whilesR ct -> no_whilesR cf -> no_whilesR (IFB b THEN ct ELSE cf FI).
...
Proof.
	split; intros.
	Case "->".
		induction c. apply nw_skip. apply nw_ass.
		SCase "seq".
			inversion H. unfold andb in H1. destruct (no_whiles c1); [| inversion H1].
			apply nw_seq; [apply IHc1 | apply IHc2]. reflexivity. apply H1.
		SCase "ifb".
			inversion H. unfold andb in H1. destruct (no_whiles c1); [| inversion H1].
			apply nw_ifb; [apply IHc1 | apply IHc2]. reflexivity. apply H1.
		SCase "while (contradiction)".
			inversion H.
	Case "<-".
		induction c; try reflexivity.
		SCase "seq".
			inversion H. simpl. unfold andb.
			destruct (no_whiles c1); [apply IHc2; apply H3 | apply IHc1; apply H2].
		SCase "ifb".
			inversion H. simpl. unfold andb.
			destruct (no_whiles c1); [apply IHc2; apply H4 | apply IHc1; apply H2].
		SCase "while (contradiction)".
			inversion H.
Qed.
(** [] *)
...
Proof.
	...
	Case "skip".
		intros. apply ex_intro with (st). apply E_Skip.
	Case "assignment".
		intros. apply ex_intro with (update st id (aeval st a)). apply E_Ass. reflexivity.
	Case "seq".
		intros.
		specialize (IHno_whilesR1 st).
		inversion IHno_whilesR1.
		specialize (IHno_whilesR2 x).
		inversion IHno_whilesR2.
		apply ex_intro with (x0).
		apply E_Seq with (x).
		apply H1. apply H2.
	Case "ifb".
		intros.
		specialize (IHno_whilesR1 st). specialize (IHno_whilesR2 st).
		inversion IHno_whilesR1. inversion IHno_whilesR2.
		destruct (beval st b) eqn:Hbeval.
		apply ex_intro with (x). apply E_IfTrue. apply Hbeval. apply H1.
		apply ex_intro with (x0). apply E_IfFalse. apply Hbeval. apply H2.
Qed.
(** [] *)
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | (SLoad ident)::prog' =>
    s_execute st ((st ident)::stack) prog'
  | SPlus::prog' =>
    match stack with
...
Lemma l : forall (e : aexp) (st : state) (instr : list sinstr) (stack : list nat),
  s_execute st stack ((s_compile e)++instr) = s_execute st ((aeval st e)::stack) instr.
Proof.
  intro.
  induction e; try reflexivity; simpl; intros;
    repeat rewrite app_ass with (n := instr); simpl;
    [rewrite IHe1 with (instr:= (s_compile e2 ++ SPlus :: instr));
     rewrite IHe2 with (instr:= (SPlus :: instr));
     rewrite plus_comm
    |rewrite IHe1 with (instr := (s_compile e2 ++ SMinus :: instr));
     rewrite IHe2 with (instr := (SMinus :: instr))
    |rewrite IHe1 with (instr := (s_compile e2 ++ SMult :: instr));
     rewrite IHe2 with (instr := (SMult :: instr));
     rewrite mult_comm
    ]; try reflexivity.
Qed.
...
Proof.
  intros.
  induction e; simpl; try repeat rewrite l; simpl;
    [| | rewrite plus_comm | | rewrite mult_comm]; try reflexivity.
Qed.
...
Tactic Notation "ceval_cases" tactic(first) ident(c) :=
  first;
  [ Case_aux c "E_Skip"
   | Case_aux c "E_Break" | Case_aux c "E_Ass" | Case_aux c "E_Seq"
   | Case_aux c "E_If_True" | Case_aux c "E_If_False" | Case_aux c "E_Seq_Break"
   | Case_aux c "E_While_Break" | Case_aux c "E_While_False" | Case_aux c "E_While_True"
  ].
...
Proof.
  ...
  destruct c; try reflexivity;
    (inversion H; subst; [inversion H2 | inversion H5; reflexivity]).
Qed.
...
Proof.
  intros.
  destruct c; intros; inversion H0; subst;
    (apply E_While_Break; [apply H | apply H0]).
Qed.
...
Proof.
  ...
  apply IHceval2; [apply HeqW | apply H0].
Qed.
...
Proof.
  ...
  generalize dependent st2.
  induction H; intros.
  Case "E_Skip". inversion H0; subst; split; reflexivity.
  Case "E_Break". inversion H0; subst; split; reflexivity.
  Case "E_Ass". inversion H1; subst; split; reflexivity.
  Case "E_If_True".
    inversion H1; subst.
    apply IHceval, H9.
  ...
  Case "E_Seq".
    inversion H1; subst.
    specialize (IHceval1 _ _ H4).
    destruct IHceval1; subst.    
    apply IHceval2, H8.
    specialize (IHceval1 _ _ H7).
    destruct IHceval1.
    congruence.
  Case "E_Seq_Break".
    inversion H0; subst.
    specialize (IHceval _ _ H3); subst.
    destruct IHceval.
    congruence.
    apply IHceval.
    assumption.
  Case "E_While_Break".
    inversion H1; subst.
    specialize (IHceval _ _ H8).
    destruct IHceval.
    split; [ assumption | reflexivity ].
    congruence.
    specialize (IHceval _ _ H5).
    destruct IHceval.
    congruence.
  Case "E_While_False".
      inversion H0; subst.
  ...
  Case "E_While_True".
      inversion H2; subst.
      specialize (IHceval1 _ _ H9).
      destruct IHceval1.
      congruence.
      congruence.
      specialize (IHceval1 _ _ H6); destruct IHceval1; subst.
      specialize (IHceval2 _ _ H10); assumption. 
Qed.
